...
The actual content (`llms.txt`, `*.md`) is generated by the backend task `backend/generate_llm_content.py`. This script runs daily (or manually) and uploads the fresh content to the `divemap-prod` R2 bucket. The worker then serves whatever is in that bucket.
