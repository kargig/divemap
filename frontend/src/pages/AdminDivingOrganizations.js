...
const AdminDivingOrganizations = () => {
  ...
  const deleteOrgMutation = useMutation(
    (id) => api.delete(`/api/v1/diving-organizations/${id}`),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['admin-diving-organizations']);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Manage diving certification organizations and their details</p>
      </div>
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left">
                  <input
                    type="checkbox"
                    checked={selectedItems.size === (organizations?.length || 0) && organizations?.length > 0}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          {org.logo_url ? (
                            <img className="h-10 w-10 rounded-full object-cover" src={org.logo_url} alt={org.name} />
                          ) : (
