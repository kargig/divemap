...
const AdminTags = () => {
  ...
  const updateTagMutation = useMutation(({ id, data }) => api.put(`/api/v1/tags/${id}`, data), {
    onSuccess: () => {
      queryClient.invalidateQueries(['admin-tags']);
  ...
  const deleteTagMutation = useMutation(id => api.delete(`/api/v1/tags/${id}`), {
    onSuccess: () => {
      queryClient.invalidateQueries(['admin-tags']);
  ...
  const handleMassDelete = () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${selectedItems.size} tag(s)?\n\n${itemNames.join('\n')}`
      )
    ) {
  ...
  return (
    <>
      <SEO title='Divemap - Admin - Tags' description='Divemap Admin Dashboard' />
                ...
                <tr>
                  <th className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                    <input
                      ...
                      checked={
                        selectedItems.size ===
                          (tags?.filter(tag => tag.dive_site_count === 0).length || 0) &&
                        (tags?.filter(tag => tag.dive_site_count === 0).length || 0) > 0
                      }
                    ...
                    <td className='px-6 py-4 whitespace-nowrap text-sm font-medium'>
                      <div className='flex space-x-2'>
                        ...
                        <button
                          ...
                          disabled={tag.dive_site_count > 0}
                          title={
                            tag.dive_site_count > 0
                              ? 'Cannot delete tag that is associated with dive sites'
                              : 'Delete tag'
                          }
                        >
