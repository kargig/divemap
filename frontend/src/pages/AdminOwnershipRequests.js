...
const AdminOwnershipRequests = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(['ownership-requests'], () => getOwnershipRequests(), {
    retry: 3,
    retryDelay: 1000,
  });
  ...
  const approvalMutation = useMutation(
      ...
      approveDivingCenterOwnership(divingCenterId, { approved, reason }),
    {
      ...
      onError: _error => {
        toast.error('Failed to process ownership request');
      },
    }
  );
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {requests.map(request => (
            <div key={request.id} className='bg-white rounded-lg shadow-md p-6'>
              <div className='flex items-start justify-between mb-4'>
                <div>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(request.ownership_status)}`}
                    >
              ...
              {request.ownership_status === 'approved' && (
                <div className='space-y-3'>
                  <p className='text-sm text-gray-600'>This diving center has an approved owner.</p>
                  <div className='flex gap-2'>
                    <button
                      onClick={() => setSelectedRequest(request)}
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-lg p-6 max-w-md w-full mx-4'>
            <div className='flex items-center justify-between mb-4'>
              <h3 className='text-lg font-semibold text-gray-900'>Review Ownership Claim</h3>
              <button
            ...
            <div className='mb-4'>
              ...
              <p className='text-sm text-gray-600'>
                Status:{' '}
                {selectedRequest.ownership_status === 'claimed' ? 'Pending Approval' : 'Approved'}
              </p>
            </div>
