...
const WeatherConditionsCard = ({ windData, loading }) => {
  ...
  const formatValue = (val, unit) => (val !== null && val !== undefined ? `${val.toFixed(1)} ${unit}` : 'N/A');
    ...
    <Row gutter={[16, 16]} className="pt-2">
      ...
      <Col span={12}>
        <Statistic 
          ...
          precision={1} 
          suffix="m" 
          valueStyle={{ color: data.wave_height > 1.5 ? '#cf1322' : data.wave_height > 0.8 ? '#d48806' : '#3f8600' }}
        />
      </Col>
      <Col span={12}>
        <Statistic 
        ...
        <Descriptions size="small" column={1} bordered>
          ...
          <Descriptions.Item label="Swell">
            {data.swell_wave_height !== null ? (
              <span>
                {formatValue(data.swell_wave_height, 'm')} from {formatWindDirection(data.swell_wave_direction).cardinal} ({formatValue(data.swell_wave_period, 's')})
              </span>
            ) : 'N/A'}
          </Descriptions.Item>
      ...
      <Col span={24}>
        <Descriptions size="small" column={1} bordered>
          <Descriptions.Item label="Direction">
            {data.wind_direction_10m !== null ? formatWindDirection(data.wind_direction_10m).full : 'N/A'}
          </Descriptions.Item>
        </Descriptions>
      </Col>
    </Row>
    ...
    <div className="bg-white p-4">
      <Tabs 
        ...
        items={[
          { 
            key: 'marine', 
            label: <span><Waves className="w-4 h-4 inline mr-1"/>Marine</span>, 
            children: marineContent 
          },
          { 
            key: 'wind', 
            label: <span><Wind className="w-4 h-4 inline mr-1"/>Wind</span>, 
            children: windContent 
          }
        ]} 
      />
      <div className="mt-4 text-xs text-gray-500 text-right">
        Source: <a href="https://open-meteo.com" target="_blank" rel="noopener noreferrer" className="hover:text-blue-600 underline">Open-Meteo</a>
      </div>
    </div>
  );
};
