...
/**
 * AdminDiveRoutesTable - TanStack Table implementation for Dive Routes
 */
const AdminDiveRoutesTable = ({
  data = [],
  columns,
  pagination,
  onPaginationChange,
  ...
  const table = useReactTable({
    ...
    getRowId: row => row.id.toString(), // Use actual dive route ID for row selection
    state: {
      pagination: {
                ...
                <tr>
                  ...
                  >
                    <div className='flex flex-col items-center justify-center'>
                      ...
                      <p className='text-gray-500 text-lg font-medium'>No dive routes found</p>
                ...
                table.getRowModel().rows.map((row, index) => (
                  ...
                  >
                    {row.getVisibleCells().map(cell => {
                      ...
                      // Allow wrapping for name column (description)
                      const shouldWrap = columnId === 'name' || columnId === 'description';
                      return (
                        <td
                          key={cell.id}
                ...
                <div key={row.id} className='p-4 hover:bg-gray-50'>
                  ...
                  <div className='grid grid-cols-2 gap-2 mt-3 text-xs'>
                    ...
                    <div>
                      ...
                      <span className='text-gray-900'>{route.creator?.username || 'Unknown'}</span>
                    </div>
                    <div>
                      <span className='text-gray-500'>Dive Site:</span>{' '}
                      <span className='text-gray-900'>{route.dive_site?.name || 'Unknown'}</span>
                    </div>
                    <div>
                      <span className='text-gray-500'>Type:</span>{' '}
                      <span className='text-gray-900'>{getRouteTypeLabel(route.route_type)}</span>
                    </div>
                  </div>
                </div>
              );
      ...
      <div className='flex flex-col sm:flex-row justify-between items-center gap-4'>
        ...
        {pagination.totalCount !== undefined && (
          <div className='text-sm text-gray-600'>
            ...
            {pagination.totalCount} dive routes
          </div>
        )}
...
AdminDiveRoutesTable.propTypes = {
...
export default AdminDiveRoutesTable;
