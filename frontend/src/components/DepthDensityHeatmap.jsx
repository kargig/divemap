    ...
    <div className='mt-6'>
      ...
      <div className='flex overflow-x-auto pb-4'>
        <div className='flex min-w-max'>
          ...
          <div className='flex flex-col gap-1 pr-2 mt-[40px]'>
            {AVG_DEPTH_BINS.map(label => (
              <div
                key={label}
                className='h-6 flex items-center justify-end text-[10px] text-gray-500 w-12 text-right'
              >
                {label}
              </div>
            ))}
          ...
          <div>
            ...
            <div className='flex gap-1 mb-2'>
              {MAX_DEPTH_BINS.map(label => (
                <div key={label} className='w-6 relative h-8'>
                  <span className='absolute bottom-0 left-1/2 text-[10px] text-gray-500 origin-bottom-left -rotate-45 whitespace-nowrap'>
                    {label}
                  </span>
                </div>
              ))}
                ...
                <div key={maxBin} className='flex flex-col gap-1'>
                  {AVG_DEPTH_BINS.map(avgBin => {
                    ...
                    return (
                      <div
                        key={`${maxBin}-${avgBin}`}
                        className={`w-6 h-6 rounded-sm ${isImpossible ? 'bg-gray-50/30' : getColor(count)} transition-all hover:ring-2 hover:ring-blue-400 group relative cursor-help`}
                        title={
                          count > 0
            ...
            {/* X-Axis Legend */}
            <div className='mt-2 text-center text-[10px] text-gray-400'>Maximum Depth (m)</div>
          </div>
