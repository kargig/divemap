#!/usr/bin/env python3
"""
Script to update dive sites with country and region data using the reverse geocoding API.

This script can update a single dive site by ID or all dive sites that have coordinates
but are missing country/region information.

Usage:
    python update_dive_site_locations.py [--dive-site-id ID] [--base-url URL] [--username USER] [--password PASS] [--dry-run] [--force]
...
Examples:
    # Update a specific dive site
    python update_dive_site_locations.py --dive-site-id 123

    # Update all dive sites missing country/region data
    python update_dive_site_locations.py

    # Use custom base URL
    DIVEMAP_BASE_URL=https://api.divemap.com python update_dive_site_locations.py

    # Dry run mode - preview changes without applying them
    python utils/update_dive_site_locations.py --dry-run

    # Force update all dive sites with coordinates
    python utils/update_dive_site_locations.py --force

    # Handle rate limits with custom retry settings (optimized for sliding window)
    python utils/update_dive_site_locations.py --max-retries 5 --base-wait-time 180

    # Debug mode with rate limit handling
    python utils/update_dive_site_locations.py --debug --max-retries 3 --base-wait-time 180
    ...
    python utils/update_dive_site_locations.py --max-requests-per-minute 60 --base-wait-time 180
"""
...
class DiveSiteLocationUpdater:
    """
    Update dive sites with country and region data using reverse geocoding.
    ...
    def is_token_valid(self) -> bool:
        ...
        try:
            url = urljoin(self.base_url, "/api/v1/dive-sites/")
    ...
    def get_dive_sites(self, dive_site_id: Optional[int] = None) -> List[Dict]:
        """Get dive sites from the API."""
        try:
            ...
            if dive_site_id:
                # Get specific dive site
                url = urljoin(self.base_url, f"/api/v1/dive-sites/{dive_site_id}")
                ...
                if response.status_code == 401:
                    ...
                    if self.authenticate():
                        ...
                        return self.get_dive_sites(dive_site_id)  # Retry with new token
                    else:
                        print(f"[{get_timestamp()}] ❌ Failed to refresh token. Cannot continue.")
                        return []
                
                response.raise_for_status()
                dive_site = response.json()
                return [dive_site] if dive_site else []
            else:
                # Get all dive sites with pagination
                all_dive_sites = []
                page = 1
                page_size = 100
                ...
                while True:
                    ...
                    dive_sites = data if isinstance(data, list) else []
                    
                    if not dive_sites:
                        break
                        
                    all_dive_sites.extend(dive_sites)
                    page += 1
                    
                    # Check if we've reached the last page
                    if len(dive_sites) < page_size:
                        break
                
                return all_dive_sites
                
        except requests.exceptions.RequestException as e:
            print(f"[{get_timestamp()}] ❌ Failed to fetch dive sites: {e}")
            return []
        except Exception as e:
            print(f"[{get_timestamp()}] ❌ Unexpected error fetching dive sites: {e}")
            return []
    
    def get_reverse_geocode(self, latitude: float, longitude: float, retry_count: int = 0) -> Optional[Dict]:
        """Get country and region data using the reverse geocoding API."""
        try:
            # Ensure we have a valid token before making requests
            if not self.ensure_valid_token():
                print(f"   ❌ [{get_timestamp()}] Cannot proceed without valid authentication")
            ...
            url = urljoin(self.base_url, "/api/v1/dive-sites/reverse-geocode")
            params = {
                "latitude": latitude,
                "longitude": longitude
            }
    ...
    def update_dive_site(self, dive_site_id: int, country: str, region: str, retry_count: int = 0) -> bool:
        """Update a dive site with new country and region data."""
        try:
            if self.dry_run:
                print(f"📝 [DRY RUN] Would update dive site {dive_site_id}: country='{country}', region='{region}'")
                return True
            ...
            if response.status_code == 401:
                ...
                if self.authenticate():
                    ...
                    return self.update_dive_site(dive_site_id, country, region, retry_count)  # Retry with new token
                else:
                    print(f"   ❌ [{get_timestamp()}] Failed to refresh token. Cannot continue.")
                    return False
            ...
            if response.status_code == 429:
                ...
                if retry_count >= self.max_retries:
                    print(f"   ❌ Rate limit exceeded after {self.max_retries} retries for dive site {dive_site_id}")
                    return False
                ...
                time.sleep(wait_time)
                self.last_rate_limit_time = time.time()
                return self.update_dive_site(dive_site_id, country, region, retry_count + 1)  # Retry with incremented count
            
            response.raise_for_status()
            
            print(f"✅ Updated dive site {dive_site_id}: country='{country}', region='{region}'")
            return True
            ...
            print(f"   [{get_timestamp()}] ❌ Unexpected error updating dive site {dive_site_id}: {e}")
            return False
    
    def needs_update(self, dive_site: Dict) -> bool:
        """Check if a dive site needs country/region update."""
        # Check if dive site has coordinates
        if not dive_site.get("latitude") or not dive_site.get("longitude"):
            return False
        
        # Check if country or region is missing
        country = dive_site.get("country")
        region = dive_site.get("region")
        
        return not country or not region
    
    def has_existing_data(self, dive_site: Dict) -> bool:
        """Check if a dive site already has country and region data."""
        country = dive_site.get("country")
        region = dive_site.get("region")
        return bool(country and region)
    
    def should_update_existing(self, dive_site: Dict) -> bool:
        """Determine if we should update a dive site that already has data."""
        if self.force:
            return True
        
        if not self.has_existing_data(dive_site):
            return True
        
        # Interactive prompt for user decision
        dive_site_id = dive_site["id"]
        name = dive_site["name"]
        current_country = dive_site.get("country", "N/A")
        current_region = dive_site.get("region", "N/A")
        
        print(f"\n[{get_timestamp()}] 📍 Dive site {name} (ID: {dive_site_id}) already has location data:")
        print(f"   Current: country='{current_country}', region='{current_region}'")

        while True:
            response = input(f"   [{get_timestamp()}] Do you want to update this dive site? (y/n): ").strip().lower()
    ...
    def process_dive_site(self, dive_site: Dict) -> bool:
        """Process a single dive site for location update."""
        dive_site_id = dive_site["id"]
        name = dive_site["name"]
        ...
        if not self.should_update_existing(dive_site):
        ...
        if not new_country and not new_region:
            print(f"   ⚠️  Skipping - no country/region data available")
            return False
        
        # Check if we actually have new data to update
        if (new_country == current_country and new_region == current_region):
            print(f"   ✅ Already up to date")
            return True
        
        # Update the dive site
        print(f"   🔄 [{get_timestamp()}] Updating dive site...")
        success = self.update_dive_site(dive_site_id, new_country or "", new_region or "")
        if success:
            print(f"   ✅ Updated to: country='{new_country or 'N/A'}', region='{new_region or 'N/A'}'")
        else:
            print(f"   ❌ Update failed")
        
        return success
    
    def run(self, dive_site_id: Optional[int] = None) -> None:
        """Main execution method."""
        mode = "DRY RUN" if self.dry_run else "LIVE"
        print(f"[{get_timestamp()}] 🚀 Starting dive site location updates ({mode})...")
        ...
        # Get dive sites
        print(f"\n[{get_timestamp()}] 📋 Fetching dive sites...")
        dive_sites = self.get_dive_sites(dive_site_id)
        
        if not dive_sites:
            print(f"[{get_timestamp()}] ❌ No dive sites found")
            return
        
        print(f"[{get_timestamp()}] 📊 Found {len(dive_sites)} dive site(s)")
        
        # Filter dive sites that need updates
        if dive_site_id:
            # Single dive site mode - process regardless of current state
            sites_to_process = dive_sites
            print(f"[{get_timestamp()}] 🎯 Processing single dive site: {dive_site_id}")
        else:
            if self.force:
                # Force mode - process all sites with coordinates
                sites_to_process = [site for site in dive_sites if site.get("latitude") and site.get("longitude")]
                print(f"[{get_timestamp()}] 🔄 Force mode: Processing {len(sites_to_process)} dive site(s) with coordinates")
            else:
                # Normal mode - only process sites that need updates
                sites_to_process = [site for site in dive_sites if self.needs_update(site)]
                print(f"[{get_timestamp()}] 🔄 Found {len(sites_to_process)} dive site(s) that need location updates")
        
        if not sites_to_process:
            print(f"[{get_timestamp()}] ✅ No dive sites need updates")
            return
        
        # Process dive sites
        print(f"\n[{get_timestamp()}] 🔄 Processing {len(sites_to_process)} dive site(s)...")
        successful_updates = 0
        failed_updates = 0
        
        for i, dive_site in enumerate(sites_to_process, 1):
            print(f"\n[{get_timestamp()}] [{i}/{len(sites_to_process)}]", end="")
            if self.process_dive_site(dive_site):
        ...
        print(f"   Total processed: {len(sites_to_process)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update dive sites with country and region data using reverse geocoding",
    ...
    parser.add_argument(
        "--dive-site-id", "-i",
        type=int,
        help="Update only the specified dive site ID"
    )
    ...
    parser.add_argument(
        "--dry-run", "-d",
        action="store_true",
        help="Dry run mode: fetch country/region data but don't make actual updates"
    )
    
    parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Force update all dive sites with coordinates, even if they already have country/region data"
    )
    ...
    updater = DiveSiteLocationUpdater(args.base_url, args.username, args.password, args.dry_run, args.force, args.debug)
