...
# Focused performance testing script for dive routes
# Creates 100 routes, tests performance, then cleans up
...
echo -e "${BLUE}=== Focused Performance Testing (100 routes) ===${NC}"
echo
...
echo -e "${YELLOW}[3] Creating 100 test routes...${NC}"
CREATED_ROUTES=()
FAILED_COUNT=0
...
for i in $(seq 1 100); do
    ...
    # Progress indicator every 20 routes
    if (( i % 20 == 0 )); then
        echo -e "${BLUE}Progress: $i/100 routes created${NC}"
    fi
done
...
if [[ ${#CREATED_ROUTES[@]} -eq 0 ]]; then
    echo -e "${RED}No routes created, aborting performance test${NC}"
    exit 1
fi
...
# Test 2: Route export performance
echo -e "${BLUE}Test 2: Route export performance${NC}"
...
# Test 3: Frontend performance (simulated)
echo -e "${BLUE}Test 3: Frontend route loading (simulated)${NC}"
echo "Frontend performance would be tested with Playwright MCP"
echo "This would measure page load time with ${#CREATED_ROUTES[@]} routes"
...
for route_id in "${CREATED_ROUTES[@]}"; do
    ...
    # Progress indicator every 25 deletions
    if (( DELETED_COUNT % 25 == 0 )); then
...
if (( $(echo "$API_TIME < 1.0" | bc -l) )); then
    echo -e "API listing: ${GREEN}✓ PASS${NC} (< 1.0s)"
else
    echo -e "API listing: ${RED}✗ FAIL${NC} (>= 1.0s)"
fi

if (( $(echo "$EXPORT_TIME < 0.5" | bc -l) )); then
    echo -e "Route export: ${GREEN}✓ PASS${NC} (< 0.5s)"
else
    echo -e "Route export: ${RED}✗ FAIL${NC} (>= 0.5s)"
fi
